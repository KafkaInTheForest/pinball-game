...
void init_ball(ball_info *p){
    ...
    p->dy = p->max_s - p->dx;
   //p->s = 4;
    fb_draw_circle(p->x,p->y,p->radius,FB_COLOR(255,255,255));
    return;
}

void move(ball_info *p,status_info *ps,position *pb){
    printf("x:%d,y:%d\n",p->x,p->y);
...
void init_board(position *p){
    p->x = rand()%904 + 60;
    fb_draw_rect(p->x-board_len/2,board_height,board_len,10,FB_COLOR(255,255,255));
    return;
}
